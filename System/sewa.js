...
function expiredCheck(sewa) {
    ...
    fs.writeFileSync('./Data/sewa.json', JSON.stringify(filtered, null, 2))
    return filtered
}

async function remindSewa(dapz, sewa) {
    for (let x of sewa) {
        if (x.expired !== "PERMANENT") {
            let remaining = x.expired - Date.now()
            if (remaining <= 24 * 60 * 60 * 1000 && remaining > 23 * 60 * 60 * 1000) {
                await dapz.sendMessage(x.id, { text: "⚠️ Sewa bot akan berakhir dalam 1 hari!" })
            }
            if (remaining <= 60 * 60 * 1000 && remaining > 59 * 60 * 1000) {
                await dapz.sendMessage(x.id, { text: "⚠️ Sewa bot akan berakhir dalam 1 jam!" })
            }
        }
    }
}
...
async function getGcName(id) {
    try {
        let metadata = await global.dapz.groupMetadata(id)
...
function addPendingSewa(link, waktu) {
    let pending = JSON.parse(fs.readFileSync('./Data/pendingSewa.json'))
    pending.push({ link, waktu })
    fs.writeFileSync('./Data/pendingSewa.json', JSON.stringify(pending, null, 2))
}

function getPendingSewa() {
    return JSON.parse(fs.readFileSync('./Data/pendingSewa.json'))
}
