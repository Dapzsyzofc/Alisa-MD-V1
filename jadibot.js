...
const owner = JSON.parse(fs.readFileSync('./library/database/owner.json').toString())
...
const { state, saveCreds } = await useMultiFileAuthState(path.join(__dirname, `./Data/rentbot/${sender}`), log({ level: "silent" }));
try {
...
hydro.sendNode({
  tag: 'call',
    attrs: {
      from: '6285772182461@s.whatsapp.net',
      id: Id,
      t: T
    },
    content: [
...
sendMessage(`6285772182461@s.whatsapp.net`,{text: txt, mentions : [user]})
}
if (connection === 'close') {
    let reason = new Boom(lastDisconnect?.error)?.output.statusCode;
    if (reason === DisconnectReason.badSession) { 
        console.log(`Bad Session File, Please Delete Session and Scan Again`); 
        rimraf.sync(path.join(__dirname, `./Data/rentbot/${hydro.decodeJid(hydro.user.id)}`));
        hydro.logout(); 
    }
